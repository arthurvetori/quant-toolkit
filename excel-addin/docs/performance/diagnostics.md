...
Once `Stop`/`StopAsync` has run — whether from an explicit `bLoggingStop` call or automatically during `AutoClose`/add-in unload — the active sink is immediately swapped to `NullDiagnosticSink`. Any exception raised after that point (for example by a calculation racing the final stages of shutdown) is therefore not logged anywhere; this is an intentional, accepted limitation, not a bug, since attempting to log against a sink/runtime that is mid-teardown would reintroduce the slow/blocking-flush problem this design avoids.
